...
const Hyperspeed = ({ effectOptions = DEFAULT_EFFECT_OPTIONS }: { effectOptions?: any }) => {
  ...
  useEffect(() => {
    ...
    const distortions: any = {
        ...
        getJS: (progress: number, time: number) => {
          ...
          const distortion = new THREE.Vector3(
            Math.cos(progress * Math.PI * uFreq.x + time) * uAmp.x -
              Math.cos(0.02 * Math.PI * uFreq.x + time) * uAmp.x,
            nsin(progress * Math.PI * uFreq.y + time) * uAmp.y -
              nsin(0.02 * Math.PI * uFreq.y + time) * uAmp.y,
            nsin(progress * Math.PI * uFreq.z + time) * uAmp.z -
              nsin(0.02 * Math.PI * uFreq.z + time) * uAmp.z
          );
        ...
        getJS: (progress: number, time: number) => {
          ...
          const distortion = new THREE.Vector3(
            ...
            Math.sin(progress * Math.PI * uFreq.y + Math.PI / 2 + time) * uAmp.y -
              Math.sin(0.02 * Math.PI * uFreq.y + Math.PI / 2 + time) * uAmp.y,
            0
          );
        ...
        getJS: (progress: number, time: number) => {
          ...
          const distortion = new THREE.Vector3(
            Math.sin(progress * Math.PI * uFreq.x + time) * uAmp.x -
              Math.sin(0.0125 * Math.PI * uFreq.x + time) * uAmp.x,
            Math.sin(progress * Math.PI * uFreq.y + time) * uAmp.y -
              Math.sin(0.0125 * Math.PI * uFreq.y + time) * uAmp.y,
            0
          );
      ...
      turbulentDistortionStill: {
        ...
        getDistortion: `
          ...
          float getDistortionX(float progress){
            return (
              cos(PI * progress * uFreq.x) * uAmp.x +
    ...
    class App {
      ...
      setSize(width: number, height: number, updateStyles = true) {
        if (width <= 0 || height <= 0) {
          this.hasValidSize = false;
          return;
        }
